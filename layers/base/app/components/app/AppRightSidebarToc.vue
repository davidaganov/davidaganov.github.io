...
const tocLinks = computed(() => {
  ...
  const links = page?.body?.toc?.links || page?.toc?.links
  return Array.isArray(links) ? links : []
})

const activeTocIndex = computed(() => {
  if (!activeTocId.value) return 0
  const idx = tocLinks.value.findIndex((l: any) => String(l.id) === activeTocId.value)
  return idx >= 0 ? idx : 0
})
...
const setupTocObserver = async () => {
  ...
  const ids = tocLinks.value.map((l: any) => String(l.id))
  const els = ids
...
watch(
  () => tocLinks.value.map((l: any) => String(l.id)).join(","),
  () => setupTocObserver(),
  ...
  <div class="rounded-xl border border-white/5 bg-white/3 p-4">
    ...
    >
      ...
      <a
        ...
        :class="String(link.id) === activeTocId ? 'bg-white/5 text-white' : ''"
        :href="`#${link.id}`"
        :key="link.id"
      >
        {{ link.text }}

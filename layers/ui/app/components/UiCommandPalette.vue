...
const scrollRootElement = (): HTMLElement | null => {
...
const scrollActiveIntoView = () => {
  nextTick(() => {
    const root = scrollRootElement()
    if (!root) return

    const wrapper = root.querySelector(".simplebar-content-wrapper")
    ...
    if (activeRect.top < wrapRect.top) {
      wrapper.scrollTop -= wrapRect.top - activeRect.top
    } else if (activeRect.bottom > wrapRect.bottom) {
      wrapper.scrollTop += activeRect.bottom - wrapRect.bottom
    }
  })
}
...
watch(isOpen, async (val) => {
  ...
  const simplebar = listRef.value as { SimpleBar?: { recalculate: () => void } } | null
...
<template>
  <ClientOnly>
    ...
    >
      <template #content>
        <div
          class="max-h-[calc(100dvh-2rem)] overflow-hidden bg-white shadow-2xl outline-none dark:bg-black/90 dark:shadow-purple-500/10"
          role="presentation"
          @keydown="onKeydown"
        >
          <label
            class="flex shrink-0 cursor-text items-center gap-3 border-b border-black/5 px-5 py-3.5 dark:border-white/10"
          ...
          <div
            aria-hidden="true"
            class="hidden shrink-0 items-center justify-between border-t border-black/5 bg-black/3 px-4 py-2 text-xs text-gray-600 md:flex dark:border-white/10 dark:bg-white/6 dark:text-gray-400"
          >

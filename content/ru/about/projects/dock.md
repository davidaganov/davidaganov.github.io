...
- **Панель инспектора** для каждого проекта — вкладки **Скрипты**, **ENV** и **Пакеты**:
  - запуск / остановка / перезапуск скриптов, избранное, свой порядок;
  - `.env` с несколькими вариантами значений, копирование активного env, генерация `.env.example`;
  - пресеты версий зависимостей в `package.json`;
  - drag-and-drop порядка скриптов, переменных и пакетов (только в UI Dock);
- **Git** — выбор ветки, pull, fetch; статусы в списке проектов;
- **Встроенный терминал** с вкладками сессий и SSE-логами;
- **Скрытие / возврат** проектов без удаления из `projects.json`;
...
Локальный файл `dock-config.json` хранит пути, настройки UI и состояние инспектора по проектам (порядок скриптов, ENV / пакетов, пресеты версий). Пример — в [репозитории](https://github.com/davidaganov/dock/blob/main/dock-config.example.json).

Порядок отображения в Dock не меняет `.env` и не переставляет ключи в `package.json`. Активные значения ENV и выбранные версии пакетов записываются в файлы проекта при редактировании или переключении варианта.

...
API-клиент и организация запросов — отдельная большая тема. Если коротко: всё живёт в `services/`, ни один компонент не знает про `fetch` напрямую, а вся типизация сосредоточена в доменных модулях. Подробно я разобрал это в статье [«Чистый API-клиент»](/blog/clean-api-client).
